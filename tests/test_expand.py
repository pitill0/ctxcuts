...
def test_expand_invocation_renders_generic_variables(tmp_path: Path) -> None:
    config_dir = tmp_path / ".ctxcuts"
    contexts_dir = config_dir / "contexts"
    contexts_dir.mkdir(parents=True)
    ...
    (contexts_dir / "review.md").write_text(
        """# Review Mode
...
""",
    ...
    assert "Area: auth" in expanded.content
    assert "Risk: high" in expanded.content
    assert "Variables:" in expanded.content
    assert "- area: auth" in expanded.content
    assert "- risk: high" in expanded.content


def test_expand_invocation_renders_generic_variable_defaults(tmp_path: Path) -> None:
    config_dir = tmp_path / ".ctxcuts"
    contexts_dir = config_dir / "contexts"
    contexts_dir.mkdir(parents=True)
...
""",
    ...
    assert "Area: general" in expanded.content
    assert "Risk: unknown" in expanded.content
    assert "Variables: none" in expanded.content
